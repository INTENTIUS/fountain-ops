...
| `verify-conversation EMAIL [MODE]` | Makes a throwaway agent, opens one conversation, asserts the event stream, and tears both down even when an assertion fails. `MODE` is `plumbing` (default), `strict` or `fixture`; `strict` refuses spritzer's interpreter, and `fixture` runs fountain's ACP fixture runtime on a spritzer pod (container mode) or a wisp sprite, with `acpFixtureUserId`, asserts a completed turn and reads its artifact back from the sprite. Needs a verified account and `$FOUNTAIN_PASSWORD` in the environment rather than on the command line. See [The data plane](/fountain-ops/reference/data-plane/) |
