...
| `image` | an image reference | `ghcr.io/binarybourbon/fountain:v0.16.0` | Pinned, not a floating tag. The version is written once, in `src/lib/fountain-image.ts`; the weekly `image-pin` workflow opens a pull request when upstream is ahead of it |
