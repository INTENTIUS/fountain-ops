...
e2e-k8s/pipeline.ts  the kubernetes-target loop, on merges and on demand
image-pin/pipeline.ts  the weekly check that fountain's pin is the current release
workflows/shared.ts  pins the workflows share
...
src/
  ...
  lib/fountain-image.ts  the fountain image pin, written once
