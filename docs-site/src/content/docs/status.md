...
| Completing a turn, against the emulated data plane | **Every `just e2e`, on a spritzer pod.** spritzer `0.6.0` in container mode runs each sprite as a pod, and fountain `v0.21.0`'s deterministic ACP fixture runtime, enabled for the e2e account, completes a turn in one: `end_turn`, with the artifact it wrote read back out of the pod by `kubectl exec`. By hand, the same through chant's `fountainApply` of an Environment and a `sandbox_mode: persistent` Agent. The fixture is not a model; the `claude` runtime starts in the pod too and stops at model selection without an inference credential |
...
| | |
|---|---|
| `networking_type: limited` on spritzer | **Fails provisioning.** fountain rejects spritzer 0.6.0's answer to the network policy call (`{:network_policy, {:invalid, {:http, 200, …}}}`), with an empty allowlist or one host. `unrestricted` provisions. Upstream of this repo |
