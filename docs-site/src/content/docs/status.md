...
| Completing a turn, against the emulated data plane | **Every `just e2e`, on a spritzer pod.** spritzer `0.6.1` in container mode runs each sprite as a pod, and fountain `v0.21.0`'s deterministic ACP fixture runtime, enabled for the e2e account, completes a turn in one: `end_turn`, with the artifact it wrote read back out of the pod by `kubectl exec`. The agent runs on a `networking_type: limited` Environment with an empty allowlist, the Box default, and the network stage must finish `done` (spritzer `0.6.1`; `0.6.0` failed it, spritzer#26). By hand, the same through chant's `fountainApply` of an Environment and a `sandbox_mode: persistent` Agent. The fixture is not a model; the `claude` runtime starts in the pod too and stops at model selection without an inference credential |
...
| | |
|---|---|
| Egress limits on spritzer | spritzer stores a `networking_type: limited` Environment's policy and fountain provisions against it, but nothing enforces it: a limited sandbox on the emulator reaches any host. Enforcement needs a real data plane |

The turn row spent `v0.16.0` and the first days of `v0.21.0` here, as "the
handshake, not the turn": spritzer `0.5.0`'s interpreter answered ACP
`initialize` with `-32601`. Container mode is what moved it back, not a change
to the interpreter, which still stops there. The older evidence, 34 of 34
conversations completed at `fountain v0.6.1` + `spritzer 0.5.0`, was over a
spawn path that no longer exists.
[#67](https://github.com/INTENTIUS/fountain-ops/issues/67) holds the history of
getting this row wrong.
...
| `dataPlane=wisp` | Checked against the real host `https://wisp.widgets.wtf` from k3d, 2026-09-25: `verify-conversation ... fixture` completed a persistent agent's turn (`end_turn`) on a wisp sprite in a limited Environment, read its artifact back through wisp's filesystem API, and reset the sandbox, deleting the sprite. No `PUBLIC_URL` callback was needed. `strict` with a real inference credential has not been run there ([#121](https://github.com/INTENTIUS/fountain-ops/issues/121), [the data plane](/fountain-ops/reference/data-plane/#checked-against-wispwidgetswtf)) |
