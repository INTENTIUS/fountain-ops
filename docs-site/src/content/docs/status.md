...
| Completing a turn | **Every time, and it stopped being a race.** 34 of 34 conversations completed at `fountain v0.6.1` + `spritzer 0.5.0`, including batches fired back to back — the pacing that used to fail most. The prompt is written as stdin and comes back on stdout, so the round trip is real, not just an exit code. What completes is the **echo** — the emulator working as designed, which is why `just verify-conversation strict` refuses to run against it. Three fixes closed it: [fountain#603](https://github.com/BinaryBourbon/fountain/issues/603) (a lost stdin write crashed the ConversationServer, and its restart orphaned the turn behind a reattach), [spritzer#19](https://github.com/INTENTIUS/spritzer/pull/19) (an unupgraded GET on the exec path answered `426` instead of the session list), and [spritzer#20](https://github.com/INTENTIUS/spritzer/pull/20) (an unrecognised command now holds its exec session open until stdin EOF). [#67](https://github.com/INTENTIUS/fountain-ops/issues/67) holds the history of getting this wrong |
...
Nothing, at these pins. The last entry — completing a turn — moved up to
